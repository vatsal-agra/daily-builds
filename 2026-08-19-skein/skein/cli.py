"""Skein CLI.
    ...
    skein shuffle-proof [--trials N] [--seed N] [--script "..."]
    skein demo
"""
...
def cmd_shuffle_proof(args: argparse.Namespace) -> int:
    ...
    for _ in range(args.edits):
        sim.random_edit()
    print(f"# Generated {len(sim.all_ops)} ops across {len(site_ids)} sites (oracle text: {convergence.compute_oracle_text(sim.all_ops)!r})")
...
def cmd_demo(args: argparse.Namespace) -> int:
    ...
    print(f"  concurrently, alice types '!' and bob types '?' at the same position (not yet delivered)")
    sim.drain()
    ...
    print(f"  {shuffle_result['trials'] - len(shuffle_result['mismatches'])}/{shuffle_result['trials']} random orders converged to {shuffle_result['baseline']!r}")
